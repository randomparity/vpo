...
class TestGetFileIdsByPathPrefix:
    ...
    def test_returns_empty_list_when_no_matches(self, db_conn, insert_test_file):
        """Returns empty list when no files match."""
        insert_test_file(id=1, path="/other/path/movie.mkv", filename="movie.mkv")

        result = get_file_ids_by_path_prefix(db_conn, "/media/")

        assert result == []

    def test_returns_matching_file_ids(self, db_conn, insert_test_file):
        """Returns IDs of files under the path prefix."""
        file1_id = insert_test_file(
            id=1, path="/media/movies/movie1.mkv", filename="movie1.mkv"
        )
        file2_id = insert_test_file(
            id=2, path="/media/movies/movie2.mkv", filename="movie2.mkv"
        )
        insert_test_file(id=3, path="/other/show.mkv", filename="show.mkv")

        result = get_file_ids_by_path_prefix(db_conn, "/media/movies/")

        assert set(result) == {file1_id, file2_id}

    def test_includes_subdirectories_by_default(self, db_conn, insert_test_file):
        """Includes files in subdirectories by default."""
        file1_id = insert_test_file(
            id=1, path="/media/movies/movie.mkv", filename="movie.mkv"
        )
        file2_id = insert_test_file(
            id=2, path="/media/movies/action/action.mkv", filename="action.mkv"
        )
    ...
    def test_excludes_subdirectories_when_requested(self, db_conn, insert_test_file):
