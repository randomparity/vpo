...
class TestFileOperationContextFromFilePath:
    ...
    def test_creates_context_from_database(
        self, db_conn, test_file, insert_test_file, insert_test_track
    ):
    ...
    def test_tracks_have_database_ids(
        self, db_conn, test_file, insert_test_file, insert_test_track
    ):
        """Tracks returned have their database IDs populated."""
        file_id = insert_test_file(path=str(test_file), extension="mkv")
        track_id_1 = insert_test_track(
            file_id=file_id, track_index=0, track_type="video", codec="h264"
        )
        track_id_2 = insert_test_track(
            file_id=file_id,
            track_index=1,
            track_type="audio",
            codec="aac",
...
class TestFileOperationContextFromFileInfo:
    ...
    def test_creates_context_from_file_info(
        self, db_conn, test_file, insert_test_file, insert_test_track
    ):
        """Creates context from FileInfo with DB lookup for ID."""
        file_id = insert_test_file(
            path=str(test_file), extension="mkv", container_format="mp4"
        )
        insert_test_track(
            file_id=file_id, track_index=0, track_type="video", codec="h264"
        )
    ...
    def test_gets_tracks_from_database(
        self, db_conn, test_file, insert_test_file, insert_test_track
    ):
    ...
    def test_uses_file_info_container_format(
        self, db_conn, test_file, insert_test_file
    ):
        """Uses container_format from FileInfo, not database."""
        # Database has "mkv"
        insert_test_file(path=str(test_file), extension="mkv", container_format="mkv")
        db_conn.commit()
...
class TestOperationContextProtocol:
    ...
    def test_implements_protocol_properties(
        self, db_conn, test_file, insert_test_file, insert_test_track
    ):
        """FileOperationContext has all required protocol properties."""
        file_id = insert_test_file(path=str(test_file), extension="mkv")
        insert_test_track(
            file_id=file_id, track_index=0, track_type="audio", codec="aac"
        )
        db_conn.commit()
    ...
    def test_protocol_typing(self, db_conn, test_file, insert_test_file):
