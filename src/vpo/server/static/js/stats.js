 ...
 */
    function init() {
        ...
        // Load distribution once (doesn't depend on time filter)
        loadDistribution()
 ...
 */
    ...
    /**
 * Load library distribution data (called once, not on filter change)
 */
    async function loadDistribution() {
        try {
            var distributionRes = await fetch('/api/stats/library-distribution')
            try {
            ...
            console.warn('Error loading library distribution:', err)
            state.libraryDistribution = null
        }
    }
 ...
 */
    async function loadStats() {
        ...
        // Determine group_by based on time filter for trends
            ...
            // Fetch all data in parallel (distribution loaded separately)
            ...
            var recentRes = results[1]
            var policiesRes = results[2]
            var trendsRes = results[3]
            var libraryRes = results[4]
            ...
            try {
                ...
                console.warn('Failed to parse library trends response')
                state.libraryTrends = []
            }
            state.error = null
 ...
 */
    function renderStats() {
        ...
        var totalTracks = summary.total_audio_removed + summary.total_subtitles_removed + summary.total_attachments_removed
 ...
 */
    function renderLibraryCharts() {
        ...
        var renderLineChart = window.VPOCharts.renderLineChart
        var data = state.libraryTrends
        ...
        if (elements.chartLibraryFiles) {
            var fileData = data.map(function (s) {
                return {
        ...
        if (elements.chartLibrarySize) {
            var sizeData = data.map(function (s) {
                return {
                    date: s.snapshot_at.slice(0, 10),
 ...
 */
    function renderCompositionCharts() {
        ...
        var renderPieChart = window.VPOCharts.renderPieChart
        var data = state.libraryDistribution
 ...
 */
    ...
    // Check if charts module is available
        ...
        var renderLineChart = window.VPOCharts.renderLineChart
        var renderBarChart = window.VPOCharts.renderBarChart
        var renderGauge = window.VPOCharts.renderGauge
        ...
        if (elements.chartTrend && state.trends && state.trends.length > 0) {
            var trendData = state.trends.map(function (t) {
                return {
                    date: t.date,
        ...
        // Sort policies by size saved and take top 5
            var policyData = state.policies.slice()
                .sort(function (a, b) { return b.total_size_saved - a.total_size_saved })
                .slice(0, 5)
                .map(function (p) {
                    return {
        ...
        // avg_savings_percent is already a percentage (e.g., 25.5 for 25.5%)
            var savingsPercent = state.summary.avg_savings_percent || 0
 ...
 */
    function renderPoliciesTable() {
        var tbody = elements.policiesBody
        tbody.innerHTML = ''
 ...
 */
    async function showDetailModal(statsId) {
        ...
        try {
            var res = await fetch('/api/stats/' + statsId)
            if (!res.ok) {
            ...
            if (retryBtn) {
                retryBtn.addEventListener('click', function () { showDetailModal(statsId) })
            }
        } finally {
 ...
 */
    function renderDetailContent(detail) {
        var savingsPercent = detail.size_before > 0
        ...
        if (detail.actions && detail.actions.length > 0) {
            ...
            for (var i = 0; i < detail.actions.length; i++) {
                var action = detail.actions[i]
                var trackInfo = action.track_type
                    ? ' (' + escapeHtml(action.track_type) + (action.track_index !== null ? ' #' + action.track_index : '') + ')'
                    : ''
                var statusClass = action.success ? 'stats-action-success' : 'stats-action-error'
                html +=
