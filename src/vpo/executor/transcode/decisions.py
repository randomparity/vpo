...
def should_transcode_video(
    ...
    if policy.target_video_codec:
        ...
        if current_codec and not video_codec_matches(current_codec, target_codec):
            ...
            reasons.append(
                f"Codec {current_codec} does not match target {target_codec}"
            )
            logger.debug(
    ...
    if max_dims and current_width and current_height:
        ...
        if current_width > max_width or current_height > max_height:
            ...
            reasons.append(
                f"Resolution {current_width}x{current_height} exceeds "
                f"{max_label} max (scaling to {target_width}x{target_height})"
            )
