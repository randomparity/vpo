...
from video_policy_orchestrator.plugin.version import PLUGIN_API_VERSION, is_compatible
...
class PluginLoader:
    ...
    def load_plugin(
        ...
        # Get actual name from validated plugin
        if isinstance(plugin_obj, type):
            instance = plugin_obj()
        else:
            instance = plugin_obj
        ...
        # Create and register
        loaded = create_loaded_plugin(plugin_obj, source, path)
    ...
    def _check_acknowledgment(self, name: str, path: Path) -> bool:
        ...
        from video_policy_orchestrator.db.models import is_plugin_acknowledged
